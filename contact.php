...
if (isset($_POST['submit1'])) {
    unset($_POST['submit1']);
$err_val = array("error", "nameof_project", "raddress", "fchief_contact0", "pcontact_detail", "created_date");

   
    foreach ($_POST as $key => $value) {
        $data[$key] = addslashes(trim($value));
    }
    $allowedExts = array("jpeg", "png", "jpg", "JPEG", "PNG", "JPG","doc","docx","pdf");
...
$data['user_agent'] = $ua['name'] . "-" . $ua['version'] . "-" . $ua['platform'];
   ...
   $data['device'] = $device;
   $id = $con->insert("digital_india4", $data);
    if (is_numeric($id)) {
      $con->close_con();
        ?>
      ...
      $n = 0;
        ...
        $id = $con->insert("digital_india4", $err_val1);
        $con->close_con();
        echo "not Submitted";
    }
}
